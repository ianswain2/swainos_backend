...
def call_refresh_rpc(base_url: str, api_key: str) -> dict:
    endpoint = f"{base_url}/rpc/refresh_travel_trade_rollups_v1"
    headers = {
        "apikey": api_key,
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed refreshing travel-trade rollups: HTTP {exc.code} {details}. "
            "Ensure migration 0054 (and prior travel-trade migrations) has been applied."
        ) from exc
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Refresh travel trade agent/agency rollups.")
    parser.add_argument(
        "--env-file",
