...
def build_agency_payload(row: Dict[str, str]) -> Dict[str, Any]:
    ...
    return {
        ...
        "agency_name": normalize_text(pick(row, "agency_name", "Name")),
        "agency_code": normalize_text(pick(row, "agency_code", "IATA_Number__c")),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Upsert agencies via Supabase REST API.")
    parser.add_argument("csv_path", help="Path to agencies CSV file")
    ...
    endpoint = f"{supabase_url.rstrip('/')}/rest/v1/agencies?on_conflict=external_id"
    headers = {
    ...
    with open(args.csv_path, "r", encoding="utf-8-sig", newline="") as csv_file:
        ...
        for _ in range(args.start_row):
            next(reader, None)
        rows = (build_agency_payload(row) for row in reader)
    ...
    print(f"Agency upsert complete. Rows uploaded: {uploaded_rows}")
