...
def build_payment_payload(row: Dict[str, str]) -> Dict[str, Any]:
    ...
    return {
        ...
        "notes": row.get("notes"),
        "received_at": normalize_datetime(row.get("received_at")),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Upsert customer payments via Supabase REST API.")
    parser.add_argument("csv_path", help="Path to customer payments CSV file")
    ...
    endpoint = f"{supabase_url.rstrip('/')}/rest/v1/customer_payments?on_conflict=external_id"
    headers = {
    ...
    with open(args.csv_path, "r", encoding="utf-8", newline="") as csv_file:
        ...
        for _ in range(args.start_row):
            next(reader, None)
        rows = (build_payment_payload(row) for row in reader)
