...
from src.schemas.revenue_bookings import CashFlowFilters, DepositSummary
...
async def deposits_summary(
    filters: CashFlowFilters = Depends(get_deposit_filters),
    service: RevenueBookingsService = Depends(get_revenue_bookings_service),
) -> ResponseEnvelope[List[DepositSummary]]:
    start_date, end_date = parse_time_window(filters.time_window)
    data = await run_in_threadpool(
        service.get_deposit_summary, start_date, end_date, filters.currency_code
    )
